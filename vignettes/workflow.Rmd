...
# SyncER ships the Example dataset inside the installed package. Below we copy it
# into a writable folder of your choice and run the whole workflow there, so that
# all age-depth modelling and SyncER output are created in your own location and
# the installed package is never modified. Change `syncer_wd` to any path you like.
syncer_wd <- file.path(path.expand("~"), "SyncER_example")
dir.create(syncer_wd, recursive = TRUE, showWarnings = FALSE)

# Copy the bundled input file (record_data_input.xlsx) and the Bacon .out files
...
The setup chunk above already copied the bundled Example dataset from the installed package (`system.file("extdata", package = "SyncER")`) into `syncer_wd`, the writable working directory you chose. Call `syncer_setup()` to point *SyncER* at that folder for the rest of this document and to create the `SyncER_outputs` folder there. Note that `SyncER_outputs` only holds *SyncER*'s own results (plots, statistics, the exported `out_data_ages.xlsx`) -- the *rbacon*/*rplum* input and output folders (named after your records, e.g. `core1`) live in the working directory itself, following the folder structure that *rbacon*/*rplum* expect. When applying this to your own data, simply place your own `record_data_input.xlsx` in `syncer_wd` instead (and skip the copy step in the setup chunk). This is an excel file with all information for each of the records you would like to be considered. Each sheet represents a single record, for which information on the depth, age & error for each dated sample and the record top is given. Also include the radiocarbon calibration curve that is required *(0=no calibration, calendar ages; 1=IntCal20; 2=Marine20; 3=SHCal20)*. Additionally, the depths of the considered event deposits needs to be included. You are free to give the input depths as event-free depth or total depth.
...
list2env(expand_horizon_groups(horizon_groups), environment()) # derives event_types, isochrons, test_events, isochron_groups, and test_horizon_groups
```
...
[^1]: If you do not want to make use of the built-in compatibility with *rbacon or rplum*, skip the remainder step 2 and set up (in a different folder) the input data as desired by your age-depth modelling software (e.g., OxCal, Chronomodel - do not forget to add the relevant citations to your work). Create the necessary age-depth models, and compile an `out_data_ages.xlsx` file that is saved in the `SyncER_outputs` folder inside your working directory (`syncer_wd`). The `out_data_ages.xlsx` file should be structured as followed:
    ...
    : *Structure of the `out_data_ages.xlsx` file*

    Each column should contain the age information for a single event horizon. Each row should have one possible age, and ideally at least a thousand ages resulting from different age simulations are available (i.e. the basis for a PDF of an event age). The name of each event horizon column should be unique, and corresponding to the names in `record_data_input.xlsx`. If generic names were used (e.g., *synchronous*)*,* you could add the depth of that event in your record to avoid confusion (e.g., *synchronous_35*).

    Finally, make sure to load the file into R using the following code:

    synced_suffix \<- ""\
    event_ages \<- read_from_excel(synced=synced_suffix)

```{r bacon-setup}
input_record_data <- load_excel_data(file_name = file_name)
...
The code below includes some standard settings and automatically determines the ages of all depths until the deepest depth given in the `record_data_input.xlsx` file (be it a sample or an event deposit). If you need specific settings for a certain record (e.g., instantaneous deposits in case your input file contains total depth and not event-free depth), adjust the first part of this coding block to match the *rbacon* or *rplum* command you need (details on the possible input variations can be found in their respective manuals) and make sure to uncomment this part. Otherwise, all records will be ran with the same standard settings.

Actually calling `Bacon()`/`Plum()` requires the *rbacon*/*rplum* packages to be installed and loaded (`library(rbacon)` or `library(rplum)`) -- *SyncER* does not load them for you. The bundled `Example` dataset already comes with completed Bacon runs in the `core1`-`core5` folders, so there is no need to install *rbacon*/*rplum* or (re)run the models just to follow this vignette: `run_bacon` below defaults to `FALSE`, and every `Bacon()`/`Plum()` call in the remainder of this document is wrapped in `if (run_bacon) { ... }`, so it is skipped and the existing model output is left untouched. `extract_ages()` afterwards reads directly from those `*.out` files. Set `run_bacon <- TRUE` (and load *rbacon*/*rplum*) if you want to reconstruct the age-depth models yourself, e.g. for your own dataset.

```{r bacon-toggle}
run_bacon <- FALSE # set to TRUE to actually (re)run rbacon/rplum; requires library(rbacon) or library(rplum)
```
...
Extract the ages of the event deposits in each record and export them to the `out_data_ages.xlsx` file.

```{r extract-ages}
reload_existing <- FALSE ## FALSE processes the bundled .out files and creates out_data_ages.xlsx; set TRUE on later runs to reuse an existing out_data_ages.xlsx
event_ages <- extract_ages(record_data = record_data, event_types = event_types, max_depths = max_depths, isochrons = isochrons, test_horizons = test_events, reload_existing = reload_existing, excluded_depths=event_depths, thick = 2 ## set to the thick value used in Bacon, single value or as list (for example: list("core1" = 2, "core3 = 1))
                           )
if (!reload_existing) {
  export_to_excel(event_ages)}
```
...
isochron_synchro <- compute_synchronicity(isochron_stats, isochrons, horizon_groups = isochron_groups, confidence_level = confidence_level, age_difference = age_difference)
...
For each of these methods, you can exclude a certain record from the calculations (e.g., if it has inaccurate age information) by adding the `excluded_records` argument to the `set_to_zero()` function.
...
As in step 2, actually (re)running *rbacon*/*rplum* here is gated by `run_bacon` (see step 2) as the `Example` dataset already includes completed `_synced` runs.
...
After constructing the new age-depth models, extract the ages of the different event deposits in each synchronized record and export them to the `out_data_ages_synced.xlsx` file.

```{r extract-synced-ages}
reload_existing <- FALSE ## FALSE processes the bundled _synced .out files and creates out_data_ages_synced.xlsx; set TRUE on later runs to reuse an existing file
event_ages_synced <- extract_ages(record_data = record_data, event_types = event_types, max_depths = max_depths, isochrons = isochrons, test_horizons = test_events, synced = "_synced", reload_existing = reload_existing, thick = 2)
if (!reload_existing) {
  export_to_excel(event_ages_synced, synced="_synced")}
```
...
thresholds <- calculate_validation_thresholds(adjusted_ages, age_offset=age_offset, sigma_multiplier=2)
...
isochron_synchro_synced <- compute_synchronicity(isochron_stats_synced, isochrons,confidence_level=thresholds$confidence_levels,  age_difference=thresholds$validation_thresholds, 
...
test_synchro <- compute_synchronicity(test_event_stats, test_events, confidence_level = confidence_level, age_difference = thresholds_tests, horizon_groups = test_horizon_groups)
...
adjusted_ages <- set_to_zero(test_event_stats, test_synchro, method = matching_method, horizons = "synchro-test", excluded_horizons = non_synchro_horizons, offset = age_offset, horizon_groups = test_horizon_groups)
...
non_synchro_ages <- assign_excluded_ages(adjusted_ages, non_synchro_horizons, test_event_stats)
...
event_ages_synced_1 <- extract_ages(record_data = record_data_synced_1, event_types = event_types, max_depths = max_depths, isochrons = isochrons, test_horizons = test_events, synced = "synced_1",reload_existing = FALSE, thick = 2)
export_to_excel(event_ages_synced_1, synced = "synced_1")
```

You could iteratively keep synchronizing new horizons by re-running step 6 and this step again, each time passing the latest `record_data_synced_N` into `age_model_input()` -- it creates the next numbered generation (`_synced_2`, `_synced_3`, ...) and returns the record_data to carry into the following iteration.
